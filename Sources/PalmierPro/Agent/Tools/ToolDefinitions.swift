...
enum ToolDefinitions {
                ...
                properties: [
                    "source": [
                        ...
                        "properties": [
                            ...
                            "path": ["type": "string", "description": "Absolute local file or directory path, readable by CreatorStudio Editor. Files are referenced in place and must remain available. A directory is imported recursively and its folder structure is replicated as media folders."],
    ...
    static let manageProject = AgentTool(
        name: .manageProject,
        description: "List, open, create, or close CreatorStudio Editor projects for this MCP session. Set `action` to: `list` for known projects plus session-active and visible state; `open` with a name, id from list, or .palmier path; `create` with an optional name and initial fps/aspectRatio/quality; or `close` to save and close the session project, optionally targeting another open project by name/id/path. Opening or creating changes only this session's target. Closing always completes a final save first. This tool never deletes projects or files.",
        inputSchema: objectSchema(
            properties: [
    ...
    static var mcpServer: [AgentTool] { all.filter { $0.name != .upscaleMedia } + [manageProject] }
    static var inAppAgent: [AgentTool] { all.filter { $0.name != .upscaleMedia } + [readSkill] }
